...
const BadgePanoramicMonitor: React.FC = () => {
  ...
  const handleCityClick = (city: any) => {
    ...
    // Mock data generation for stores in the selected city
    const mockStores = [
      { name: `${city.name}高新区旗舰店`, total: 45, online: 44, status: 'Normal' },
      { name: `${city.name}锦江区体验中心`, total: 32, online: 30, status: 'Warning' },
      { name: `${city.name}武侯区4S店`, total: 28, online: 20, status: 'Alert' },
      { name: `${city.name}天府新区展厅`, total: 15, online: 15, status: 'Normal' },
    ];
    setCityStores(mockStores);
  };
    ...
    <div className={`bg-[#0b1121] text-white overflow-hidden flex flex-col relative font-sans ${isFullScreen ? 'h-screen rounded-none' : 'min-h-[calc(100vh-2rem)] rounded-xl'}`}>
      ...
      <div className="flex-1 relative z-10 grid grid-cols-12 gap-6 p-6 overflow-hidden">
        ...
        <div className="col-span-6 relative flex flex-col items-center justify-center p-4">
           ...
           <div className="relative w-full h-full max-h-[600px] aspect-[4/3]">
              ...
              {CITY_CLUSTERS.map((city, i) => (
                 ...
                 >
                    ...
                    {/* City Name - Clickable with Hover Effect */}
                    <div className={`mt-1 text-[10px] whitespace-nowrap font-medium transition-all duration-300 group-hover:text-blue-400 group-hover:scale-110 group-hover:font-bold ${i < 5 ? 'text-white font-bold text-shadow-glow' : 'text-slate-500'}`}>
                       {city.name}
                    </div>
           ...
           {/* Drill Down Modal Overlay - Fixed Position */}
           {selectedCity && (
             <div className="absolute top-10 right-10 w-80 bg-slate-900/95 backdrop-blur-xl border border-blue-500/30 rounded-2xl shadow-2xl p-5 flex flex-col animate-fade-in z-50 overflow-hidden">
               <div className="flex justify-between items-start mb-6">
                 <div>
                   <h3 className="text-2xl font-bold text-white flex items-center gap-2">
                     <MapPin size={24} className="text-blue-400" /> {selectedCity.name}
                   </h3>
                   <p className="text-xs text-slate-400 mt-1">区域数据看板</p>
                 </div>
                 <button onClick={closeDrillDown} className="p-1 hover:bg-white/10 rounded-full text-slate-400 hover:text-white transition-colors">
                   <X size={20} />
                 </button>
               </div>
               ...
               <div className="grid grid-cols-2 gap-4 mb-6">
                 <div className="bg-slate-800/60 p-4 rounded-xl text-center border border-slate-700/50">
                   <p className="text-xs text-slate-400 mb-1">活跃工牌</p>
                   <p className="text-2xl font-bold text-blue-400 font-mono">{selectedCity.count * 15}</p>
                 </div>
                 <div className="bg-slate-800/60 p-4 rounded-xl text-center border border-slate-700/50">
                   <p className="text-xs text-slate-400 mb-1">风险预警</p>
                   <p className={`text-2xl font-bold font-mono ${selectedCity.count > 80 ? 'text-red-500' : 'text-green-500'}`}>
                     {selectedCity.count > 80 ? 5 : 0}
                   </p>
                 </div>
               </div>

               <h4 className="text-sm font-bold text-blue-100 mb-4 flex items-center gap-2 border-l-4 border-blue-500 pl-2">
                 <Server size={16} className="text-blue-400" /> 重点门店监控
               </h4>
               
               <div className="flex-1 overflow-y-auto space-y-3 max-h-60 pr-1 custom-scrollbar">
                 {cityStores.map((store, i) => (
                   <div key={i} className="p-4 bg-slate-800/40 rounded-xl border border-slate-700/50 hover:bg-slate-800/60 transition-colors group">
                     <div className="flex justify-between items-center mb-3">
                       <span className="text-sm font-bold text-slate-200 group-hover:text-white transition-colors">{store.name}</span>
                     ...
                     <div className="flex justify-between text-xs text-slate-400 mb-2">
                       <span>工牌: {store.total}</span>
                       <span>在线: <span className="text-slate-200">{store.online}</span></span>
                     </div>
                     <div className="w-full h-1.5 bg-slate-700 rounded-full overflow-hidden">
                       <div 
                         className={`h-full ${store.status === 'Alert' ? 'bg-red-500' : store.status === 'Warning' ? 'bg-orange-500' : 'bg-blue-500'}`} 
                         style={{ width: `${(store.online/store.total)*100}%` }}
                       ></div>
                     </div>
                   </div>
                 ))}
               ...
               <button className="mt-4 w-full py-3 bg-blue-600/20 hover:bg-blue-600/40 text-blue-300 rounded-xl text-sm font-bold transition-all border border-blue-500/30 hover:border-blue-500/60 shadow-[0_0_15px_rgba(37,99,235,0.1)]">
                 查看完整报表
               </button>
             </div>
           )}
      ...
      <style>{`
        ...
        /* Custom Scrollbar for drilldown */
...
export default BadgePanoramicMonitor;
